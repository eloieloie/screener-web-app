...
// Session store for Cloud Functions
const MemoryStoreSession = MemoryStore(session);

// Environment variables (set in GCP Console or .env.yaml)
...
// Middleware setup
app.use(cors({
  origin: [FRONTEND_URL, 'http://localhost:5173', 'http://localhost:5174', 'http://localhost:3000', 'http://el-mac.ddns.net', 'https://el-mac.ddns.net'],
  credentials: true,
...
app.get('/', (req, res) => {
  res.json({
    message: 'Screener Backend API - Google Cloud Function',
    version: '1.0.0',
    status: 'running',
    endpoints: {
      health: '/health',
...
app.get('/api/stocks/top', requireAuth, checkService, async (req, res) => {
  try {
    ...
    return app._router.handle({ ...req, method: 'POST', url: '/api/stocks/multiple' }, res);
  } catch (error) {
...
functions.http('screenerApi', app);
